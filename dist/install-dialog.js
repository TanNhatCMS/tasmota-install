...
export class EwtInstallDialog extends LitElement {
            ...
            : html `
              <div>
                ...
                if (this._manifest.home_assistant_domain) {
                    window.open(`https://my.home-assistant.io/redirect/config_flow_start/?domain=${this._manifest.home_assistant_domain}`, "_blank");
                }
                this._busy = false;
            }}
                ></ewt-button>
    ...
    async _updateSsids(tries = 0) {
        ...
        if (ssids.length === 0 && tries < 3) {
            this.logger.log(`SSID scan returned empty, scheduling retry ${tries + 1}/3`);
            setTimeout(() => {
                if (this._state === "PROVISION") {
    ...
    async _initialize(justInstalled = false, skipImprov = false) {
        ...
        if (this._improvChecked) {
            this.logger.log(`Improv already checked - ${this._improvSupported ? "supported" : "not supported"}, skipping re-test`);
        ...
        if (inBootloaderMode) {
            this.logger.log("Device is in BOOTLOADER mode - switching to FIRMWARE mode for Improv test");
            if (isUsbJtagOrOtg) {
     ...
     */
    async _switchToFirmwareMode(actionAfterReconnect = null) {
        const inBootloaderMode = this.esploader.chipFamily !== null;
        if (!inBootloaderMode) {
        ...
        // CRITICAL: Save parent loader
        const loaderToSave = this._espStub._parent || this._espStub;
        this._savedLoaderBeforeSwitch = loaderToSave;
        ...
        if (isUsbJtagOrOtg) {
            ...
            try {
                ...
                this.logger.log(`Reset worked. Expected slip Timeout read error: ${err.message}`);
            }
            // Wait for reset to complete
            await sleep(500);
            // NOW release locks AFTER reset
    ...
    async _confirmInstall() {
        ...
        this.logger.log("Preparing device for flash operations (switching to bootloader mode)...");
        try {
            await this._prepareForFlashOperations();
        }
        catch (err) {
        ...
        if (this.firmwareFile != undefined) {
            ...
            flash(async (state) => {
                ...
                if (state.state === "finished" /* FlashStateType.FINISHED */) {
                    ...
                    if (isUsbJtagOrOtg) {
                        ...
                        await this._handleFlashComplete().catch((err) => {
                            this.logger.error(`Post-flash cleanup failed: ${(err === null || err === void 0 ? void 0 : err.message) || err}`);
                            this._state = "ERROR";
                            this._error = `Post-flash cleanup failed: ${(err === null || err === void 0 ? void 0 : err.message) || err}`;
                        });
                    }
                    else {
                        // For non-USB-JTAG/OTG, run async (no need to wait)
                        void this._handleFlashComplete().catch((err) => {
                            this.logger.error(`Post-flash cleanup failed: ${(err === null || err === void 0 ? void 0 : err.message) || err}`);
                            this._state = "ERROR";
                            this._error = `Post-flash cleanup failed: ${(err === null || err === void 0 ? void 0 : err.message) || err}`;
                        });
            ...
            }, loaderToUse, this.logger, this.manifestPath, this._installErase, new Uint8Array(0), this.baudRate).catch((flashErr) => {
    ...
    async _flashFilebuffer(fileBuffer) {
        ...
        }, loaderToUse, this.logger, this.manifestPath, this._installErase, fileBuffer, this.baudRate).catch((flashErr) => {
            this.logger.error(`Flash error: ${flashErr.message || flashErr}`);
            this._state = "ERROR";
            this._error = `Flash failed: ${flashErr.message || flashErr}`;
            this._busy = false;
    ...
    async _handleSelectNewPort() {
        ...
        try {
            ...
            if (globalThis.requestSerialPort) {
                ...
                newPort = await globalThis.requestSerialPort((msg) => this.logger.log("[WebUSB]", msg));
            }
            else {
                // Desktop Web Serial
    ...
    async _testImprov(timeout = 1000, skipReset = false) {
        var _a, _b;
        ...
        try {
            ...
            this.logger.log(`Port info: VID=0x${(_a = portInfo.usbVendorId) === null || _a === void 0 ? void 0 : _a.toString(16).padStart(4, "0")}, PID=0x${(_b = portInfo.usbProductId) === null || _b === void 0 ? void 0 : _b.toString(16).padStart(4, "0")}`);
            ...
            if (!skipReset) {
                ...
                try {
                    ...
                    this.logger.log("Waiting for firmware running to be ready for Improv test...");
                    await sleep(500);
                }
                catch (resetErr) {
            ...
            improvSerial.addEventListener("error-changed", () => this.requestUpdate());
            ...
            this.logger.log("Waiting for firmware to get valid IP address (checking every 500ms, max 10 seconds)...");
            ...
            if (!hasValidIp) {
                this.logger.log(`Timeout after ${maxWaitTime / 1000} seconds - continuing with current URL: ${improvSerial.nextUrl || "undefined"}`);
            }
            ...
            this.logger.log(`Improv state: ${improvSerial.state}, nextUrl: ${improvSerial.nextUrl || "undefined"}`);
        }
        catch (err) {
